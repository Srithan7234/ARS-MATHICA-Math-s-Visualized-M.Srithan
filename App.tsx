...
import { FractalApplications } from './components/FractalApplications';
...
import { 
  ...
  List, RotateCw, Check, Camera, Move3d, ZoomIn, ZoomOut, Maximize, Music, Mic, MicOff, Tv, Globe
} from 'lucide-react';
...
const App: React.FC = () => {
  // Global Nav State
  const [activeTab, setActiveTab] = useState('EXPLORER'); // EXPLORER, ART, DETECT, COMMUNITY, APPLICATIONS
  ...
  const NavItem = ({ icon: Icon, label, id, onClick, highlight = false }: { icon: any, label: string, id?: string, onClick?: () => void, highlight?: boolean }) => (
    <button 
        onClick={onClick ? onClick : () => setActiveTab(id || 'EXPLORER')}
    ...
    >
        <div className={`p-3 rounded-xl transition-all ${activeTab === id ? 'bg-purple-500/10 ring-1 ring-purple-500/50' : (highlight ? 'bg-teal-500/10 ring-1 ring-teal-500/30 group-hover:bg-teal-500/20' : 'group-hover:bg-white/5')}`}>
            <Icon size={24} strokeWidth={1.5} className={highlight ? 'text-teal-400' : ''} />
        </div>
        <span className={`text-[9px] font-bold text-center leading-none tracking-wide ${highlight ? 'text-teal-400/80' : ''}`}>{label}</span>
    ...
    <div className="flex h-screen bg-black text-gray-300 font-sans selection:bg-purple-500/30 selection:text-white overflow-hidden relative">
      ...
      <aside className="w-[100px] bg-black border-r border-white/10 flex flex-col items-center py-6 gap-2 shrink-0 z-50 shadow-2xl">
          ...
          <div className="mt-auto w-full flex flex-col items-center gap-2">
            <NavItem icon={Globe} label="APPLICATIONS" highlight={true} onClick={() => setIsApplicationsOpen(true)} />
      ...
      <FractalApplications isOpen={isApplicationsOpen} onClose={() => setIsApplicationsOpen(false)} />
                                ...
                                <div className="mt-3 flex flex-col gap-3">
                                  <div className="flex items-center justify-between">
                                    ...
                                    {!musicMode && (
                                      <div className="flex bg-black/40 rounded-lg p-0.5 border border-white/5">
                                        <button 
                                          onClick={() => setAudioSource('BROWSER_TAB')}
                                          className={`px-2 py-1 rounded text-[9px] font-bold transition-all ${audioSource === 'BROWSER_TAB' ? 'bg-purple-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}
                                        >
                                          Tab
                                        </button>
                                        <button 
                                          onClick={() => setAudioSource('MICROPHONE')}
                                          className={`px-2 py-1 rounded text-[9px] font-bold transition-all ${audioSource === 'MICROPHONE' ? 'bg-purple-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}
                                        >
                                          Mic
                                        </button>
                                      </div>
                                    ...
                                    {musicMode && (
                                      <span className="text-[10px] text-purple-400 font-bold uppercase">{audioSource === 'BROWSER_TAB' ? 'TAB AUDIO' : 'MIC AUDIO'}</span>
                                    )}
                                  </div>
                                  ...
                                  <div className="flex items-center justify-between">
                                    ...
                                    <button 
                                      ...
                                      className={`flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-bold transition-all border ${musicMode ? 'bg-purple-600 text-white border-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.4)]' : 'bg-gray-800 text-gray-500 border-white/5 hover:text-gray-300'}`}
                                    >
                                      {musicMode ? (audioSource === 'BROWSER_TAB' ? <Tv size={10}/> : <Mic size={10}/>) : <MicOff size={10}/>}
                                      {musicMode ? 'ACTIVE' : 'START'}
                                    </button>
                                  </div>
                                </div>
                                ...
                                {musicMode && (
                                  <div className="animate-in fade-in slide-in-from-top-2 duration-300 space-y-4">
                                    {audioSource === 'BROWSER_TAB' && (
                                      <div className="p-2 bg-purple-900/10 border border-purple-500/20 rounded text-[9px] text-purple-300 leading-tight">
                                        Capturing from browser tab. Ensure music is playing in shared tab.
                                      </div>
                                    )}

                                    {/* Using memoized Sub-component to prevent full app re-renders */}
                                    <VolumeIndicator />
                                    ...
                                    <div className="grid grid-cols-3 gap-1">
                                      {['GENTLE', 'MODERATE', 'INTENSE'].map(p => (
                                        <button 
                                          key={p} 
                                          onClick={() => setMusicPreset(p as MusicPreset)}
                                          className={`py-1 rounded text-[9px] font-bold border transition-all ${musicPreset === p ? 'bg-purple-600 border-purple-400 text-white shadow-sm' : 'bg-[#13141f] border-white/5 text-gray-500 hover:text-gray-400'}`}
                                        >
                                          {p}
                                        </button>
                                      ))}
                   ...
                   <div className="relative flex-1 bg-black rounded-2xl border border-white/10 overflow-hidden shadow-2xl flex items-center justify-center">
                       <div className="absolute top-0 left-0 right-0 p-4 flex justify-between items-start z-10 pointer-events-none">
                           <div className="pointer-events-auto">
                               <div className="flex items-center gap-2 text-green-400 mb-1"><TreeDeciduous size={16} /> <span className="font-bold text-sm text-gray-200">{artPattern.replace(/_/g, ' ')}</span></div>
                           </div>
                           <div className="flex items-center gap-2 pointer-events-auto">
                               ...
                               <button onClick={() => { setArtAngle(25); setArtDepth(10); setArtRatio(0.7); setArtZoom(1.0); artRef.current?.resetView(); }} className="p-2 bg-[#0F1016]/80 hover:bg-[#1F212E] rounded-lg text-gray-400 hover:text-white border border-white/5"><RefreshCw size={14} /></button>
                               <button onClick={() => setArtZoom(z => z * 1.2)} className="p-2 bg-[#0F1016]/80 hover:bg-[#1F212E] rounded-lg text-gray-400 hover:text-white border border-white/5"><ZoomIn size={14} /></button>
                               <button onClick={() => setArtZoom(z => Math.max(0.1, z / 1.2))} className="p-2 bg-[#0F1016]/80 hover:bg-[#1F212E] rounded-lg text-gray-400 hover:text-white border border-white/5"><ZoomOut size={14} /></button>
                   ...
                   <div className="bg-[#13141f] border border-white/5 rounded-xl p-4 shrink-0 space-y-5">
                       ...
                       <div className="relative">
                            <button onClick={() => setArtPatternDropdownOpen(!artPatternDropdownOpen)} className="w-full flex items-center justify-between p-3 bg-[#0a0b10] border border-white/5 rounded-lg text-sm text-gray-300 hover:text-white hover:border-white/10 transition-colors">
                                <span className="text-xs font-bold">{artPattern.replace(/_/g,' ')}</span><ChevronDown size={14} />
                            </button>
