...
/**
 * Provides mathematical insights using the complex reasoning model.
 */
...
/**
 * Analyzes fractal patterns in images using the complex reasoning model.
 */
...
/**
 * Optimizes user prompts for technical precision.
 */
export const optimizePrompt = async (originalPrompt: string): Promise<string> => {
    ...
    try {
        // Technical/precise generation tasks use 'gemini-3-pro-preview'
        const response = await ai.models.generateContent({
            model: 'gemini-3-pro-preview',
            contents: prompt,
        });
...
/**
 * Generates fractal artwork using specialized image models.
 */
export const generateFractalArt = async (prompt: string, style: string, imageSize: string, refImage?: string): Promise<string> => {
    ...
    try {
        try {
             // Upgrade to gemini-3-pro-image-preview for high quality
             ...
             // gemini-2.5-flash-image is the default image model
